...
python script/run_cpp_build.py \
    -t "项目地址:/root/project/.帮我完成该项目的cpp代码编译构建。编译构建过程中可能遇到各类问题：比如编译器版本、标准库版本、第三方库版本等，这些问题解决思路：先收集信息制定方案，方案确定后再开展具体修复工作。补充提示：该代码库是一个成熟代码库，优先从依赖问题入手，版本依赖可以根据需要调整，如果有可能尽量不要修改源代码，另外项目根目录中的Agent.md文件提供了编译安装指南。如果遇到棘手问题难以解决，search_agent具备网络检索能力，能够帮你定位问题。提示：该任务之前已经开展部分工作，现在需要继续后续工作" \
    -p ${project_id} \
